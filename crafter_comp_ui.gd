...
func _ready() -> void:
	...
	for recipe in Enums.recipes:
		if recipe.valid_machines.has(crafter_component.get_parent_GO().code_name):
			option_button.add_icon_item(recipe.icon, recipe.name)
		else:
			option_button.add_item("not avalible")
...
func _on_control_item_selected(index: int) -> void:
	if index == 0:
		...
		return
	var new_recipe = Enums.recipes[index-1]
	if new_recipe.valid_machines.has(crafter_component.get_parent_GO().code_name):
		crafter_component.recipe = new_recipe
	else: 
		crafter_component.recipe = null
